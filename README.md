...
- Rewrites page — view and edit all active DNS rewrites
- Live logs page
...
![Overview](https://github.com/user-attachments/assets/28a391bc-99f6-4bbd-90d9-aa91bfc1260e)
*Overview — sync metrics, client coverage, and recent sync history*

![Clients](https://github.com/user-attachments/assets/ee945d43-ab47-4107-9bce-c1f8d41bdb8b)
*Clients — search, filter by VLAN/device type, bulk actions*

![Client modal](https://github.com/user-attachments/assets/54fee4bf-01f7-415c-8ee2-7b2e8ad26624)
*Per-client modal — block/unblock, DNS rewrites, tag override, sync exclusion*

![Config](https://github.com/user-attachments/assets/b1f84d14-3a25-4ef5-beab-94f15b08110b)
...
Open `http://localhost:8888` and configure your UniFi and AdGuard credentials in the dashboard. No config files or environment variables needed.

---

## UniFi account setup

Create a dedicated read-only local account in UniFi to use with this tool:

1. Go to **Settings → Admins & Users → Invite Admin**
2. Set role to **Read Only**
3. Enter those credentials in the dashboard under **Configuration → Connections**
...
Detection order: `os_name` → VLAN fallback → hostname heuristics
...
You can override auto-detected tags per device directly from the dashboard.
